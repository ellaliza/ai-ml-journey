...
print(f'Student(s) with highest math score have IDs {min_math_students} with a score of {min_math[0][1]} \n\n')
...
def create_histogram(scores, axes):
    ...
    plt.hist([scores['math'], scores['science'], scores['english']], 
             bins=ranges, 
...
def create_piechart(payments, ids, axes):
    axes.pie(payments, labels = ids, colors = ['#487EB0', '#53E0BC', '#F0DF87', '#8395A7','#5281ce','#bae8ac', '#ff7582'], autopct='%1.3f%%')
    axes.set_title('Top 12 Payments')

def create_scatter(scores, axes):
    axes.scatter(scores['math'], scores['science'], color='#ff7582')
